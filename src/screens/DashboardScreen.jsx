...
import { Zap, TrendingUp, AlertTriangle, RotateCcw, Eye, EyeOff } from 'lucide-react'
        ...
        <div className="w-1/2 flex flex-col flex-shrink-0 p-4" style={{ height: '100%', paddingTop: '21px' }}>
          <div className="rounded-2xl overflow-hidden flex flex-col shadow-sm" style={{ ...card, height: '100%' }}>
            <div className="px-4 py-3 flex items-center gap-2 flex-shrink-0" style={{ borderBottom: '1px solid var(--border)' }}>
              ...
              <button onClick={() => setShowTiles(t => !t)} className="ml-auto p-1 rounded-md transition-colors"
                style={{ color: showTiles ? 'var(--muted)' : '#dc2626' }}
                title={showTiles ? 'Скрыть фон' : 'Показать фон'}>
